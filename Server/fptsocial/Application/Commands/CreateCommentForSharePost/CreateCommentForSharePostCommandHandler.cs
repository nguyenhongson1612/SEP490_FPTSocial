...
namespace Application.Commands.CreateCommentForSharePost
{
    public class CreateCommentForSharePostCommandHandler : ICommandHandler<CreateCommentForSharePostCommand, CreateCommentForSharePostCommandResult>
    {
        ...
        public async Task<Result<CreateCommentForSharePostCommandResult>> Handle(CreateCommentForSharePostCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.ParentCommentId.HasValue)
            {
                ...
                {
                    listNumber = parentComment.CommentSharePostId.ToString();
                    levelCmt = 3;
                }
            ...
            Domain.CommandModels.CommentSharePost comment = new Domain.CommandModels.CommentSharePost
            {
                CommentSharePostId = _helper.GenerateNewGuid(),
            ...
            await _context.CommentSharePosts.AddAsync(comment);
            await _context.SaveChangesAsync();

            // Map the comment to the result object
            var result = _mapper.Map<CreateCommentForSharePostCommandResult>(comment);
            ...
            // Return the result
            return Result<CreateCommentForSharePostCommandResult>.Success(result);
        }
    }
}
