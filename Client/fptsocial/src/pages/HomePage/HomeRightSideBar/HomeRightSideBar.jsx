// import { useState } from 'react'
// import { Link } from 'react-router-dom'

import { Link } from 'react-router-dom'

function HomeRightSideBar() {
  // const [isMore, setIsMore] = useState(false)
  return (
    <div className="max-h-[calc(100vh_-_55px)] w-[380px] hidden lg:!flex flex-col overflow-y-auto scrollbar-none-track text-lg font-semibold">

      <div className="ml-3 mt-8 mb-5">
        <div id="people"
          className="flex flex-col items-start "
        >
          <p className=" text-gray-500">Contacts</p>
          <Link to={`/profile?id=${'65cd438c-25c9-4642-a178-4425c2042310'}`} className="w-full px-2 py-3 hover:bg-orangeFpt hover:text-white flex items-center gap-3 cursor-pointer rounded-md">
            <img
              src="data:image/jpeg;base64,"
              className=" aspect-square rounded-[50%] object-cover w-8"
            />

            <span className="font-semibold">test use1</span>
          </Link>
          <Link to={`/profile?id=${'33424ee1-3d35-466f-b8b5-1a4753e8fbf1'}`} className="w-full px-2 py-3 hover:bg-orangeFpt hover:text-white flex items-center gap-3 cursor-pointer rounded-md">
            <img
              src="data:image/jpeg;base64,"
              className=" aspect-square rounded-[50%] object-cover w-8"
            />

            <span className="font-semibold">test use2</span>
          </Link>
        </div>
      </div>


    </div>
  )
}

export default HomeRightSideBar